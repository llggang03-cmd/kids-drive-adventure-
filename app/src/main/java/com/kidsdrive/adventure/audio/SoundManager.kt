package com.kidsdrive.adventure.audio
...
class SoundManager {
    ...
    fun playCoin() = playTone(frequency = 880f, durationMs = 90)
    fun playStar() = playTone(frequency = 1320f, durationMs = 140)
    fun playClick() = playTone(frequency = 520f, durationMs = 55)
    fun playCrash() = playNoiseBurst(durationMs = 220)
    ...
    private fun playChime() {
        try {
            val freqs = listOf(660f, 880f, 990f)
            val perNoteMs = 90
    ...
    private fun playNoiseBurst(durationMs: Int) {
        try {
            ...
            for (i in 0 until numSamples) {
                ...
                val value = noise * amplitude * Short.MAX_VALUE * 0.35
                samples[i] = value.toInt().toShort()
            }
            playSamples(samples)
        } catch (e: Exception) {
